...
def sort(stack1, stack2, stack3):
    # sort stack1 by pivot >= top element stack1
    pivot = stack1.pop()
    stack2.push(pivot)
    while(not stack1.empty()):
        temp = stack1.pop()
        stack2.push(temp) if temp >= pivot else stack3.push(temp)

    while(not stack2.empty()):
        temp = stack2.pop()
        while(not stack1.empty() and temp > stack1.peek()):
            stack2.push(stack1.pop())
        stack1.push(temp)

    while(not stack3.empty()):
        temp = stack3.pop()
        while temp > stack1.peek():
